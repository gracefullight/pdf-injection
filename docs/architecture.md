...
flowchart LR
    ...
    D --> E["injectPdf() — pdf-engine (9 modes incl. 4 round-3 probes, payloadLanguage)"]
    E --> F[output.pdf]
...
| `packages/pdf-engine` | `inspectSource()` (magic bytes, size, page count, encryption/signature detection, page geometry snapshot, `riskFlags`), `injectPdf()` (dispatches to white-text / render-mode-3 / visible-control / `xmp_only` / `unicode_tags` injectors, `payloadLanguage="ko"` via `@pdf-lib/fontkit`), `compareGeometry()`, `resolveTargetPage()`, `normalizePrompt()`, `buildManifest()`, `readXmpPayload()`, `koreanFontAvailable()`, and (for `unicode_tags`) `encodeUnicodeTags()`/`decodeUnicodeTags()`/`stripUnicodeTags()` (the Unicode Tag block codec, `unicode-tags.ts`) plus `readUnicodeTagsPayload()` (`read-unicode-tags-payload.ts` — a public-`pdf-lib`-API CMap read-back that confirms the tag-encoded payload actually landed in the output's `/ToUnicode` CMap, used as `apps/api`'s post-injection correctness gate and by `packages/benchmark`'s mock provider, since `packages/validation`'s `pdfjs-dist`-based `extractText()` structurally cannot see this mode's payload — see [`docs/validation.md`](validation.md#unicode_tags-verification-independent-of-pdfjs)). Round-3 research/diagnostic probe injectors (not production channels): `injectImageOnly()`/`injectFreetextAnnot()`/`injectAcroFormField()`/`injectInfoDict()` plus their own dedicated readers (`readStampedImagePresence()`/`readFreetextAnnotPayload()`/`readAcroFormFieldPayload()`/`readInfoDictPayload()`), each independent of `pdfjs-dist` for the same reason `readUnicodeTagsPayload()` is — see [`docs/validation.md`](validation.md#round-3-probe-modes-verification-independent-of-pdfjs). `native-canvas.ts` resolves `@napi-rs/canvas` through `pdfjs-dist`'s own module root for `injectImageOnly()`, mirroring `packages/robustness/src/native-canvas.ts` |
...
2. The background task resolves each requested `BenchmarkCondition`'s PDF via
   ...
   every injected mode (including `xmp_only`/`unicode_tags` and the 4 round-3 probe modes) re-runs `injectPdf()` with the job's own stored
...
└── <jobId>/
    ...
    ├── conditions/        # cached condition PDFs for model-test runs (<mode>.pdf, all 9 injection
    │                      # modes, incl. the 4 round-3 probes: image_only/freetext_annot/acroform_field/info_dict)
