...
| `render_mode_3` | Uses PDF text-rendering mode 3 (`3 Tr`, `TextRenderingMode.Invisible`) — text stays in the content stream but is never painted | **Experimental** | Never painted, but still discoverable by select-all/copy-paste and screen readers: drag-selecting shows bare highlight bars in an otherwise empty margin, and copying reproduces the instruction in plaintext (see [`docs/limitations.md`](docs/limitations.md#not-painted-is-not-the-same-as-not-discoverable)); some parsers/sanitizers strip render-mode-3 text entirely; provider ingestion pipelines may ignore it; extraction results are recorded explicitly (success or failure), never assumed; also detectable by independent metamorphic-detection tools (PhantomLint: 100% recall, 0.092% FPR — see [`docs/related-work.md`](docs/related-work.md#5-detectability-finding-and-what-it-implies)), by design |
