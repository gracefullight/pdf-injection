...
// the doc comment on embedKoreanFont for the full provenance and why this,
...
async function loadFontBytes(): Promise<Uint8Array> {
  const fontPath = path.join(resolveFontDir(), FONT_FILENAME);
 ...
 * composite-glyph outlines for this glyph-rich CJK font (cycles 2-3, 4 font
 ...
 *   the correct weight (paired with this file's static-Regular font — see
 *   `FONT_FILENAME`'s comment).
 ...
 *
 * `text` should be the exact instruction being drawn (the HarfBuzz subset is
 * scoped per-call to that text + printable ASCII — cheap, millisecond-level,
 * so no cross-call caching is needed).
 */
export async function embedKoreanFont(doc: PDFDocument, text: string): Promise<PDFFont> {
  ...
  const rawFontBytes = await loadFontBytes();
  ...
  try {
    ...
    throw new FontUnavailableError(
      `Failed to pre-subset the Korean CJK font with HarfBuzz: ${(err as Error).message}`,
    );
  }
  ...
  try {
    ...
    throw new FontUnavailableError(
      `Failed to embed the Korean CJK font: ${(err as Error).message}`,
    );
  }
}
