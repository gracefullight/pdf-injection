...
/**
 * PRD §26 Phase 4/5 gating — negative case: with `PS_RESEARCH_MODE` off (simulated here via
 ...
 */
test("research mode off: Submissions/Robustness tabs show the PS_RESEARCH_MODE gate message", async ({
  page,
}) => {
  ...
  await expect(submissionsGate).toContainText("PS_RESEARCH_MODE=true");
  ...
  await expect(robustnessGate).toContainText("PS_RESEARCH_MODE=true");
